...
import {
  RankingListSchema,
  type RankingList,
} from '@/features/rankings/models/ranking-list.schema';
import { RankingItemWithDetailsSchema } from '@/features/rankings/models/ranking-item.schema';
import { applyMovieFilters } from '@/shared/utils/helpers/applyMovieFilters';
...
export const rankingSessionService = {
  ...
  async getMovies(sessionId: string) {
    const { data, error } = await supabase
      .from('ranking_list_items_with_details')
      .select('*')
